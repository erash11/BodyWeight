...
# Load the dataset
file_path = r"C:\Users\Eric_Rash\OneDrive - Baylor University\AppliedPerformancProjects\CodingProjects\PythonScripts\DashDeploy\BW_061124.csv"
...
def update_graph(mode, selected_value):
    ...
    for date, weight in zip(avg_weight_by_month['YearMonth'], avg_weight_by_month['WEIGHT']):
        fig.add_trace(go.Scatter(x=[date, date + pd.DateOffset(days=30)], y=[weight, weight],
                                 ...
                                 text=[None, f'{weight:.2f}'],  # Add data labels
    ...
    fig.update_layout(
        title=f'Body Weight per Month for {selected_value} in {selected_value}',
        xaxis_title='Date',
